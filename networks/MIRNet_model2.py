...
class ChannelPool(nn.Layer):
    def forward(self, x):
        return paddle.concat((paddle.max(x,1,keepdim=True), paddle.mean(x,1,keepdim=True)), axis=1)
...
class ca_layer(nn.Layer):
    def __init__(self, channel, reduction=8, bias=True):
        ...
        self.conv_du = nn.Sequential(
                nn.Conv2D(channel, channel // reduction, 1, padding=0, bias_attr=bias),
                nn.ReLU(),
                nn.Conv2D(channel // reduction, channel, 1, padding=0, bias_attr=bias),
                nn.Sigmoid()
        )
...
class DAU(nn.Layer):
    def __init__(
        ...
        Layers_body = [conv(n_feat, n_feat, kernel_size, bias=bias), act, conv(n_feat, n_feat, kernel_size, bias=bias)]
        ...
        self.conv1x1 = nn.Conv2D(n_feat*2, n_feat, kernel_size=1, bias_attr=bias)
...
##########################################################################
##---------- Resizing Layers ----------
class ResidualDownSample(nn.Layer):
    def __init__(self, in_channels, bias=False):
        super(ResidualDownSample, self).__init__()

        self.top = nn.Sequential(nn.Conv2D(in_channels, in_channels,   1, stride=1, padding=0, bias_attr=bias),
                                nn.PReLU(),
                                nn.Conv2D(in_channels, in_channels, 3, stride=1, padding=1, bias_attr=bias),
                                nn.PReLU(),
                                downsamp(channels=in_channels,filt_size=3,stride=2),
                                nn.Conv2D(in_channels, in_channels*2, 1, stride=1, padding=0, bias_attr=bias))
...
class DownSample(nn.Layer):
    def __init__(self, in_channels, scale_factor, stride=2, kernel_size=3):
        ...
        for i in range(self.scale_factor):
            Layers_body.append(ResidualDownSample(in_channels))
...
class ResidualUpSample(nn.Layer):
    def __init__(self, in_channels, bias=False):
        super(ResidualUpSample, self).__init__()

        self.top = nn.Sequential(nn.Conv2D(in_channels, in_channels,   1, stride=1, padding=0, bias_attr=bias),
                                nn.PReLU(),
                                nn.Conv2DTranspose(in_channels, in_channels, 3, stride=2, padding=1, output_padding=1, bias_attr=bias),
                                nn.PReLU(),
                                nn.Conv2D(in_channels, in_channels//2, 1, stride=1, padding=0, bias_attr=bias))
...
class UpSample(nn.Layer):
    def __init__(self, in_channels, scale_factor, stride=2, kernel_size=3):
        ...
        Layers_body = []
        for i in range(self.scale_factor):
            Layers_body.append(ResidualUpSample(in_channels))
            in_channels = int(in_channels // stride)
...
class MSRB(nn.Layer):
    def __init__(self, n_feat, height, width, stride, bias):
        ...
        self.selective_kernel = nn.LayerList([SKFF(n_feat*stride**i, height) for i in range(height)])
    ...
    def select_up_down(self, tensor, j, k):
        if j==k:
            ...
            diff = 2 ** np.abs(j-k)
            if j<k:
                return self.up[f'{tensor.shape[1]}_{diff}'](tensor)
            else:
                return self.down[f'{tensor.shape[1]}_{diff}'](tensor)
...
class RRG(nn.Layer):
    def __init__(self, n_feat, n_MSRB, height, width, stride, bias=False):
        ...
        Layers_body = [MSRB(n_feat, height, width, stride, bias) for _ in range(n_MSRB)]
        Layers_body.append(conv(n_feat, n_feat, kernel_size=3))
        self.body = nn.Sequential(*Layers_body)
...
class MIRNet(nn.Layer):
    def __init__(self, in_channels=3, out_channels=3, n_feat=64, kernel_size=3, stride=2, n_RRG=3, n_MSRB=2, height=3, width=2, bias=False):
        ...
        self.conv_in = nn.Conv2D(in_channels, n_feat, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias_attr=bias)

        Layers_body = [RRG(n_feat, n_MSRB, height, width, stride, bias) for _ in range(n_RRG)]
        self.body = nn.Sequential(*Layers_body)

        self.conv_out = nn.Conv2D(n_feat, out_channels, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias_attr=bias)
